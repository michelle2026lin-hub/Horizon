...
读取 Horizon 生成的中文日报，调用 AI 按【完整】Melody_Script_Map_v4.0.md 逐条核对，结果推送飞书。
...
FALLBACK_SCRIPT_MAP = """
⚠️ 警告：未找到完整版 Melody_Script_Map_v4.0.md，当前为最小兜底版本，映射能力大幅受限。
请立即检查 data/Melody_Script_Map_v4.0.md 是否已上传到仓库。
"""
...
Melody_Script_Map_v4.0.md（系统唯一真理源，包含全部业务模块(16已废弃,现为17-24)、全局常量、二创规则、AI筛选宪法）
逐条核对，生成《候选工具建议书》。
{warning}
{dedup_section}
## Melody_Script_Map_v4.0.md（完整原文，唯一真理源 — 必须逐条对照，禁止凭空推荐）
...
3. **映射验证（核心步骤）**：对每个候选工具，必须在上方完整 Script Map 中找到具体的原子子步骤 ID
   ...
   如果在全部业务模块(16已废弃,现为17-24)中都找不到对应映射，直接跳过该工具，不生成建议书。
...
5. **评分**：基础分(0-10) + 加成/减成（严格按 Script Map 中"AI 筛选宪法"第7条）：
   映射到模块17 Order+Video 或 模块19 矩阵混剪日更 → +1.0（若两者都命中，只加一次，不叠加）；
   映射到模块23 实时通话翻译 → +0.5；
   映射到模块24 知识视频蒸馏 → +0.5；
   映射到"去机器味/WhatsApp筛选/竞对情报"等高痛区 → +0.5；
   映射到核心风控步骤 → -0.5；
...
`📭 今日日报无匹配新工具（已对照全部业务模块(16已废弃,现为17-24)），Melody 可跳过。`
